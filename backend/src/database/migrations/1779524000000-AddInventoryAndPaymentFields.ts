...
export class AddInventoryAndPaymentFields1779524000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."orders_paymentstatus_enum" AS ENUM('Paid', 'Unpaid')`,
    );
    await queryRunner.query(
      `ALTER TABLE "orders" ADD "paymentStatus" "public"."orders_paymentstatus_enum" NOT NULL DEFAULT 'Unpaid'`,
    );

    await queryRunner.query(
      `CREATE TYPE "public"."orders_paymentmode_enum" AS ENUM('Cash', 'UPI', 'Card', 'Net Banking')`,
    );
    await queryRunner.query(
      `ALTER TABLE "orders" ADD "paymentMode" "public"."orders_paymentmode_enum"`,
    );

    await queryRunner.query(
      `ALTER TABLE "orders" ADD "cashCollectionDetails" text`,
    );
    await queryRunner.query(
      `ALTER TABLE "orders" ADD "paymentUpdatedAt" TIMESTAMP`,
    );

    await queryRunner.query(`ALTER TABLE "orders" ADD "fulfillmentHubId" uuid`);
    await queryRunner.query(
      `ALTER TABLE "orders" ADD CONSTRAINT "FK_orders_fulfillmentHub" FOREIGN KEY ("fulfillmentHubId") REFERENCES "inventory_locations"("id") ON DELETE SET NULL`,
    );

    await queryRunner.query(`ALTER TABLE "orders" ADD "sourceId" uuid`);
    await queryRunner.query(
      `ALTER TABLE "orders" ADD CONSTRAINT "FK_orders_source" FOREIGN KEY ("sourceId") REFERENCES "order_sources"("id") ON DELETE SET NULL`,
    );

    // 6. Populate default data
    await queryRunner.query(
      `INSERT INTO "order_sources" ("name") VALUES ('WhatsApp'), ('Phone'), ('Instagram'), ('Website'), ('Walk-in') ON CONFLICT DO NOTHING`,
    );
    await queryRunner.query(
      `INSERT INTO "inventory_locations" ("name") VALUES ('Hub West'), ('Hub South') ON CONFLICT DO NOTHING`,
    );

    // 7. Map existing orders' text values to sourceId
    await queryRunner.query(
      `UPDATE "orders" SET "sourceId" = (SELECT "id" FROM "order_sources" WHERE "name" = 'Phone') WHERE "source"::text = 'Phone'`,
    );
    await queryRunner.query(
      `UPDATE "orders" SET "sourceId" = (SELECT "id" FROM "order_sources" WHERE "name" = 'WhatsApp') WHERE "source"::text = 'WhatsApp'`,
    );
    await queryRunner.query(
      `UPDATE "orders" SET "sourceId" = (SELECT "id" FROM "order_sources" WHERE "name" = 'Instagram') WHERE "source"::text = 'Instagram'`,
    );
    await queryRunner.query(
      `UPDATE "orders" SET "sourceId" = (SELECT "id" FROM "order_sources" WHERE "name" = 'Website') WHERE "source"::text = 'Website'`,
    );
    await queryRunner.query(
      `UPDATE "orders" SET "sourceId" = (SELECT "id" FROM "order_sources" WHERE "name" = 'Walk-in') WHERE "source"::text = 'Walk-in'`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `UPDATE "orders" SET "source" = 'Phone' WHERE "sourceId" = (SELECT "id" FROM "order_sources" WHERE "name" = 'Phone')`,
    );
    await queryRunner.query(
      `UPDATE "orders" SET "source" = 'WhatsApp' WHERE "sourceId" = (SELECT "id" FROM "order_sources" WHERE "name" = 'WhatsApp')`,
    );
    await queryRunner.query(
      `UPDATE "orders" SET "source" = 'Instagram' WHERE "sourceId" = (SELECT "id" FROM "order_sources" WHERE "name" = 'Instagram')`,
    );
    await queryRunner.query(
      `UPDATE "orders" SET "source" = 'Website' WHERE "sourceId" = (SELECT "id" FROM "order_sources" WHERE "name" = 'Website')`,
    );
    await queryRunner.query(
      `UPDATE "orders" SET "source" = 'Walk-in' WHERE "sourceId" = (SELECT "id" FROM "order_sources" WHERE "name" = 'Walk-in')`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "orders" DROP COLUMN "fulfillmentHubId"`,
    );

    await queryRunner.query(
      `ALTER TABLE "orders" DROP COLUMN "paymentUpdatedAt"`,
    );
    await queryRunner.query(
      `ALTER TABLE "orders" DROP COLUMN "cashCollectionDetails"`,
    );
