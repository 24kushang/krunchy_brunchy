...
export class UploadService {
  ...
  constructor(private configService: ConfigService) {
    ...
    if (credPath && fs.existsSync(credPath)) {
      try {
        ...
        this.logger.log(
          'Google Drive API client initialized successfully using keyfile.',
        );
      } catch (err) {
        this.logger.error(
      ...
      this.logger.warn(
        'Google Drive credentials keyfile not found. Falling back to Mock Upload Simulation.',
      );
    }
  }
  ...
  async uploadFile(file: Express.Multer.File): Promise<string> {
    if (this.driveClient && this.folderId) {
      try {
        this.logger.log(
          `Uploading file ${file.originalname} to Google Drive...`,
        );
        ...
        this.logger.log(
          `Successfully uploaded to Google Drive. File ID: ${response.data.id}`,
        );
        return (
          response.data.webViewLink ||
          `https://drive.google.com/open?id=${response.data.id}`
        );
      } catch (err) {
        this.logger.error(
          'Google Drive Upload failed. Falling back to simulation.',
          err.message,
        );
      }
    }
    ...
    this.logger.warn(
      `[SIMULATION] Uploading file ${file.originalname} to mock Google Drive.`,
    );
